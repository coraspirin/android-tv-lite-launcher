...
static inline std::unique_ptr<Decoder> makeDecoder(int ct, int spf, int sampleRate, int channels,
                                                   ...
                                                   bool realtimePriority, bool lowLatency) {
    if (ct == CT_ALAC) {
        // Kutu Mirror: FFmpeg (software ALAC) is removed. Only a platform ALAC
        // decoder is attempted; the mirroring path never uses ALAC.
        (void)forceSwAlac;
        ...
        log.error("ALAC init failed (no hardware decoder; software ALAC removed)");
        return nullptr;
    }
