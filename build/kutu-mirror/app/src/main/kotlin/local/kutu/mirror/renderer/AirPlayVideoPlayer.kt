package local.kutu.mirror.renderer
...
class AirPlayVideoPlayer(private val context: Context) {
    ...
    private var _startPositionSeconds = 0f
    private var _openedAsHls = false
    private var _retriedAsOtherType = false
    ...
    private val _listener = object : Player.Listener {
        override fun onPlayerError(error: PlaybackException) {
            val loc = _location
            if (loc != null && !_retriedAsOtherType) {
                // the url alone does not say whether this is HLS or a plain file: AirPlay
                // hands over UxPlay's rewritten playlist on loopback with no extension, and
                // senders also pass plain media urls. If the first guess could not be read,
                // try the other kind once before giving up.
                _retriedAsOtherType = true
                Log.w(TAG, "could not read as ${if (_openedAsHls) "HLS" else "progressive"}, retrying as the other", error)
                mainHandler.post { _open(loc, _startPositionSeconds, asHls = !_openedAsHls) }
                return
            }
    ...
    fun play(location: String, startPositionSeconds: Float) = mainHandler.post {
        ...
        _retriedAsOtherType = false
        _open(location, startPositionSeconds, asHls = _looksLikeHls(location))
    }

    /**
     * UxPlay rewrites HLS master playlists and serves them from its own loopback httpd,
     * where the url carries no `.m3u8` for ExoPlayer to infer a type from. Without this
     * hint DefaultMediaSourceFactory falls back to the progressive extractors and every
     * one of them fails to sniff a playlist.
     */
    private fun _looksLikeHls(location: String): Boolean =
        location.contains(".m3u8") ||
            location.startsWith("http://localhost:") ||
            location.startsWith("http://127.0.0.1:")

    private fun _open(location: String, startPositionSeconds: Float, asHls: Boolean) {
        ...
        player = p
        val item = MediaItem.Builder().setUri(location)
            .apply { if (asHls) setMimeType(MimeTypes.APPLICATION_M3U8) }
            .build()
        p.setMediaItem(item, (startPositionSeconds * 1000).toLong())
    ...
    private fun _stopInternal(reportStopped: Boolean) {
        if (reportStopped) _location = null
