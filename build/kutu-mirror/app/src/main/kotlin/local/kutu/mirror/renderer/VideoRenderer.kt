...
import local.kutu.mirror.renderer.DecoderSelector.Companion.videoCaps
...
class VideoRenderer(ctx: Context) {
    ...
    fun setResolution(w: Int, h: Int) = synchronized(lock) {
        ...
        // The decoder is configured for one size, and adaptive decoders are additionally
        // told that size as their maximum. Rotating the sender changes the stream's
        // dimensions mid-session, which the running decoder cannot absorb - on this box
        // it degrades the picture badly. Drop the codec; feedFrame rebuilds it at the new
        // size on the next keyframe, which the sender emits on rotation anyway.
        if (codec != null) {
            Log.i(TAG, "video size changed to ${w}x${h}, restarting decoder")
            stopCodec()
        }
    }
